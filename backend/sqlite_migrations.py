...
def _migrate_agents(cursor: sqlite3.Cursor):
    ...
    columns: list[tuple[str, str]] = [
        ...
        ("restricted_reply", "TEXT DEFAULT 'Sorry, the service is currently restricted. Please try again later.'"),
        ("last_error_code", "VARCHAR(50)"),
        ("last_error_message", "TEXT"),
        ("last_error_at", "DATETIME"),
        ("allowed_widget_origins", "TEXT"),
        ("persona_type", "VARCHAR(20) DEFAULT 'general'"),
        ("widget_title", "VARCHAR(100) DEFAULT 'AI Support'"),
        ("widget_color", "VARCHAR(20) DEFAULT '#06B6D4'"),
        (
            "welcome_message",
            "TEXT DEFAULT 'Hello! I am the Basjoo assistant. How can I help you?'",
        ),
...
def _backfill_agents(cursor: sqlite3.Cursor):
    ...
    if "widget_title" in col_names:
        cursor.execute(
            "UPDATE agents SET widget_title = 'AI Support' "
    ...
    if "welcome_message" in col_names:
        cursor.execute(
            "UPDATE agents SET welcome_message = 'Hello! I am the Basjoo assistant. How can I help you?' "
            "WHERE welcome_message IS NULL OR welcome_message = ''"
        )

    restricted_reply_default = "Sorry, the service is currently restricted. Please try again later."

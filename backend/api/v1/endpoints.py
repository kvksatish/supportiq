...
async def prepare_chat_request(
    ...
    if use_mock_llm:
        logger.info("Agent has no API Key configured, using Mock LLM service (for testing/demo only)")
    ...
    if agent_rate_limit_per_minute > 0 and request.session_id and not admin_user:
        ...
        if messages_last_minute >= agent_rate_limit_per_minute:
            limit_reply = get_restricted_reply(
                ...
                "Sorry, the service is currently restricted. Please try again later.",
            )
            logger.info(
        ...
        try:
            ...
            if kb_results:
                ...
                for i, r in enumerate(kb_results, 1):
                    source_title = r.get('source_title') or r.get('filename', 'Document')
                    source_url = r.get('source_url', '')
                    text_content = r['text']
                    if source_url:
                        texts.append(f"[Source {i}] {text_content}\n(Source: {source_title}, URL: {source_url})")
                    else:
                        texts.append(f"[Source {i}] {text_content}\n(Source: {source_title})")
                kb_context = "\n\n".join(texts)
    ...
    if kb_context:
        system_content += (
            f"\n\nHere is the relevant background information:\n\n{kb_context}\n\nPlease answer the user's question based on the above."
            "\n\nCitation note: When answering, cite sources using the format [Title](#source-N), where N is the source number."
            "Example: According to [Petking Website](#source-1), our products..."
        )
    else:
        system_content += (
...
async def chat(
    ...
    try:
        ...
        fallback = get_restricted_reply(
            _restricted_reply, "Sorry, the service is currently busy. Please try again later."
        )
        return ChatResponse(
            reply=fallback,
            sources=sources,
    ...
    if not reply or not reply.strip():
        ...
        reply = get_restricted_reply(
            _restricted_reply, "Sorry, I cannot answer this question right now. Please try rephrasing."
        )
...
async def get_chat_messages(
    ...
    - No role: returns all messages (for history restore)
    - role=assistant: returns only assistant messages (for polling admin replies)
    """
...
async def create_agent(
    ...
    agent = Agent(
        ...
        or "Hello! I am the Basjoo assistant. How can I help you?",
    )
...
async def kb_reset(
    ...
    return {
        "message": "Embedding configuration reset. All files need to be re-uploaded.",
    }
...
async def list_available_models(
    ...
    Get available models based on provider type and API Key
    """
...
async def get_tasks_status(
    ...
    Used by frontend to check if index modification operations are allowed
    """
    await require_agent_admin(db, agent_id, current_user)
...
async def test_embedding_api(
    ...
    """Test if current Embedding configuration is available"""
    agent = await require_agent_admin(db, agent_id, current_user)
...
async def test_jina_api(
    ...
    """Test if Jina Embedding API is available"""
    agent = await require_agent_admin(db, agent_id, current_user)
...
async def get_sources_summary(
    ...
    Returns total, trained, and pending counts for URLs and QA items
    """
...
async def get_public_config(
    ...
    """
    Returns public configuration including current server address and default public Widget settings.
    """
    host = (
...
async def list_sessions(
    ...
    Supports filtering by status and keyword
    """
    ...
    for session in sessions:
        ...
        items.append(
            SessionListItem(
                ...
                last_message=last_msg[:100] if last_msg else None,  # Limit length
            )
        )
...
async def send_session_message(
    ...
    """
    Human sends message to session
    """
    ...
    message = ChatMessage(
        ...
        role="assistant",
        content=content,
        sender_type="human",  # Marked as human-sent
        sender_id=str(current_user.id),  # Record sender ID
    )
    db.add(message)
...
async def refetch_urls(
    ...
    """Re-fetch URL content and index to knowledge base.

    - url_ids: List of URL IDs to re-fetch; if empty, re-fetches all pending/success URLs
    - force: Whether to force re-fetch (ignore content hash deduplication)
    """
    agent = await require_agent_admin(db, agent_id, current_user)
...
async def repair_url_indexed_status(
    ...
    """Fix the is_indexed status of URLs.

    Due to a historical bug (session isolation issue), some URLs had KbDocument processed successfully
    but the is_indexed field remained False. This endpoint batch-fixes based on KbDocument status.

    Returns fix stats: fixed (count fixed), already_correct (count already correct), errors (count with issues)
    """
    await require_agent_admin(db, agent_id, current_user)
...
async def crawl_site(
    ...
    """Crawl entire site, discover and index pages.

    - url: Starting URL
    - max_depth: Maximum crawl depth (1-5)
    - max_pages: Maximum number of pages (1-500)
    """
    import asyncio
...
async def discover_urls(
    ...
    """Discover URL subpages without immediate indexing (returns discovered URL list)."""
    await require_agent_admin(db, agent_id, current_user)
...
async def rebuild_index(
    ...
    - force: Whether to force rebuild (delete existing index and reprocess)
    """
    agent = await require_agent_admin(db, agent_id, current_user)

...
async def test_agent_name_accepts_five_chinese_characters(client):
    response = await client.post(
        "/api/v1/agents",
        json={"name": "Support Bot 1", "agent_type": "ai_clone"},
    )

    assert response.status_code == 201
    assert response.json()["name"] == "Support Bot 1"


@pytest.mark.asyncio
@pytest.mark.parametrize("name", ["AgentName12", "SupportBotAB"])

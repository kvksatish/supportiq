 ...
 * Embeddable intelligent chat component
 */
...
class BasjooWidget {
  ...
  private async loadPublicConfig() {
    ...
    try {
      ...
      if (!this.hasTitleOverride) {
        this.config.title = data.widget_title || 'AI Assistant'
      }
      if (!this.hasWelcomeMessageOverride) {
        this.config.welcomeMessage = data.welcome_message || 'Hello! How can I help you?'
      }
      this.effectiveTheme = this.getEffectiveTheme()
    } catch (error) {
  ...
  async init() {
    ...
    // On page load, start title blink to draw user attention
  ...
  private showGreetingBubble() {
    ...
    // Position above and to the side of the button
  ...
  /**
   * Start title blink notification
   */
  ...
  private getText(key: 'sendFailed' | 'networkError' | 'quotaExceeded' | 'takenOverNotice' | 'inputPlaceholder' | 'messageTooLong' | 'greetingBubble' | 'newMessage' | 'thinking' | 'references'): string {
    const texts: Record<string, string> = {
      ...
      takenOverNotice: 'Your conversation has been transferred to a human agent. Please wait for their reply.',
  ...
  /**
   * Safely escape plain text to HTML
   */
  ...
  /**
   * Start polling for new messages (messages sent by admin after takeover)
   */
...
// Export to global scope (not using export default to avoid ES Module format)

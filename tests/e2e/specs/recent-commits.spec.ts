...
test.describe('Recent commit regressions', () => {
  ...
  test('auth language switcher works before login', async ({ page }) => {
    ...
    const languageControls = page.getByRole('button').filter({ hasText: /English|EN/i });
    ...
    await page.locator('input[type="email"]').or(page.getByLabel(/email/i)).first().fill(ADMIN_EMAIL);
    await page.locator('input[type="password"]').or(page.getByLabel(/password/i)).first().fill(ADMIN_PASSWORD);

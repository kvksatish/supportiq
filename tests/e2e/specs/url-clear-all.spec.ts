...
test.describe("URL Bug Fixes", () => {
	test("Bug 1: URL status banner clears after crawl completes", async ({
		...
		const addButton = page
			.getByRole("button", { name: /single page/i })
		...
		const crawlingBanner = page
			.locator("div")
			.filter({ hasText: /Crawling in progress/i });
	...
	test("Bug 2: Clear All uses POST method and returns deleted_count", async ({
		...
		const clearAllButton = page
			.getByRole("button", { name: /Clear List/i })
			.or(page.locator("button").filter({ hasText: /Clear List/i }));
		...
		expect(alertMessage.message()).toMatch(/Successfully cleared/);
		...
		const emptyState = page
			.locator("div")
			.filter({ hasText: /No URLs|please add/i });

...
test.describe('Admin Sessions Takeover', () => {
  ...
  test('sessions page shows visitor sessions after login', async ({ page, request }) => {
    ...
    await expect(page.getByRole('heading', { name: /Sessions Center/i })).toBeVisible({ timeout: 10_000 });
    await expect(page.getByText(new RegExp(`Session #${sessionId}`))).toBeVisible({ timeout: 10_000 });
  });
});
